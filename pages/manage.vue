<template>
  <div class="manage-page">
    <v-row>
      ...
      <simplebar
        class="posts-list py-md-0 float-sm-left order-2 order-md-1"
        :class="[activePost ? 'col-md-4' : 'col-md-10']"
          ...
          <v-slide-y-transition group>
            <v-hover v-for="post in posts" :key="post._id" v-slot:default="{ hover }">
              <v-card
                ...
                @click.native="activatePost(post)"
              >
                <v-card-title class="headline">{{post.title}}</v-card-title>
                <v-card-subtitle class="font-italic">{{post.subtitle}}</v-card-subtitle>
                <v-card-text>
                  <p>{{post.content}}</p>
                  <div class="text-right">
                    <em>
                      <small>From John Leider</small>
                    </em>
                  </div>
                </v-card-text>
              </v-card>
      ...
      <simplebar
        v-if="activePost && posts.length > 0"
        cols="12"
        md="8"
        class="col col-12 col-md-8 py-md-0 order-1 order-md-2 post-manage pa-5"
        id="message-preview"
    ...
    <footer class="footer">
      ...
      <v-container fluid class="text-center py-0">
        <v-col cols="12">
          <strong>Blog Created</strong> by
          <a href="https://github.com/kathirr007">kathirr007</a>. The source
          ...
          <v-tooltip top tag="span">
            <template v-slot:activator="{ on }">
              <v-btn
                ...
                v-on="on"
                class="mr-2"
                href="https://github.com/kathirr007/my-nuxt-posts"
                target="_blank"
              >
  ...
  methods: {
    ...
    deletePost() {
      if (this.activePost) {
        ...
        this.$store
          .dispatch("posts/deletePost", this.activePost._id)
          .then(() => {
            // this.setInitialActivePost()
